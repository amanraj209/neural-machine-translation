...
class BaseModel(object):
    ...
    def __init__(self, hparams, iterator, mode, source_vocab_table, target_vocab_table,
        ...
        if self.mode == tf.contrib.learn.ModeKeys.TRAIN:
            ...
            clipped_grads, grad_norm_summary, grad_norm = model_util.gradient_clip(gradients, max_gradient_norm=hparams.max_gradient_norm)
    ...
    def _get_learning_rate_warmup(self, hparams):
        ...
        return tf.cond(self.global_step < hparams.warmup_steps,
                       ...
                       lambda: self.learning_rate, name="learning_rate_warump_cond")
    ...
    def init_embeddings(self, hparams, scope):
        self.embedding_encoder, self.embedding_decoder = model_util.create_emb_for_encoder_and_decoder(share_vocab=hparams.share_vocab,
    ...
    def train(self, sess):
        ...
        return sess.run([self.update, self.train_loss, self.predict_count, self.train_summary, self.global_step,
    ...
    def _build_encoder_cell(self, hparams, num_layers, num_residual_layers, base_gpu=0):
        return model_util.create_rnn_cell(unit_type=hparams.unit_type, num_units=hparams.num_units, num_layers=num_layers,
                                            num_residual_layers=num_residual_layers, forget_bias=hparams.forget_bias,
                                            dropout=hparams.dropout, num_gpus=hparams.num_gpus, mode=self.mode,
    ...
    def _build_decoder(self, encoder_outputs, encoder_state, hparams):
        ...
        with tf.variable_scope("decoder") as decoder_scope:
            ...
            if self.mode != tf.contrib.learn.ModeKeys.INFER:
                ...
                # apply the output_layer to all timesteps for speed
                ...
                if beam_width > 0:
                    decoder = tf.contrib.seq2seq.BeamSearchDecoder(cell=cell, embedding=self.embedding_decoder, start_tokens=start_tokens,
                                                                   end_token=end_token, initial_state=decoder_initial_state, beam_width=beam_width,
                                                                   output_layer=self.output_layer, length_penalty_weight=length_penalty_weight)
                else:
                    sampling_temperature = hparams.sampling_temperature
                    if sampling_temperature > 0.0:
                        helper = tf.contrib.seq2seq.SampleEmbeddingHelper(self.embedding_decoder, start_tokens, end_token,
                                                                          softmax_temperature=sampling_temperature, seed=hparams.random_seed)
                    else:
    ...
    def decode(self, sess):
        ...
        # make sure outputs is of shape [batch_size, time] or [beam_width, batch_size, time] when using beam search.
        if self.time_major:
    ...
    def _build_encoder(self, hparams):
        ...
        with tf.variable_scope("encoder") as scope:
            ...
            if hparams.encoder_type == "uni":
                ...
                encoder_outputs, encoder_state = tf.nn.dynamic_rnn(cell, encoder_emb_inp, dtype=dtype,
                ...
                encoder_outputs, bi_encoder_state = self._build_bidirectional_rnn(inputs=encoder_emb_inp,
                                                                                  sequence_length=iterator.source_sequence_length,
                                                                                  dtype=dtype, hparams=hparams,
