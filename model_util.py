...
__all__ = ["get_initializer", "get_device_str", "create_train_model", "create_eval_model", 
           "create_infer_model", "create_emb_for_encoder_and_decoder", "create_rnn_cell", "gradient_clip",
           "create_or_load_model", "load_model", "avg_checkpoints", "compute_perplexity"]
...
def get_initializer(init_op, seed=None, init_weight=None):
    if init_op == "uniform":
        ...
        return tf.keras.glorot_normal(seed=seed)
    elif init_op == "glorot_uniform":
        return tf.keras.glorot_uniform(seed=seed)
    else:
        raise ValueError("Unknown init op: %s" % init_op)
...
class ExtraArgs(collections.namedtuple("ExtraArgs", (
...
def create_train_model(model_creator, hparams, scope=None, num_workers=1, job_id=0, extra_args=None):
    ...
    with graph.as_default(), tf.container(scope or "train"):
        src_vocab_table, tgt_vocab_table = vocab_utils.create_vocab_tables(src_vocab_file, 
                                                                           tgt_vocab_file, 
                                                                           hparams.share_vocab)
        ...
        iterator = iterator_utils.get_iterator(src_dataset, tgt_dataset, src_vocab_table, tgt_vocab_table,
                                               batch_size=hparams.batch_size, sos=hparams.sos,
                                               eos=hparams.eos, random_seed=hparams.random_seed,
                                               num_buckets=hparams.num_buckets, src_max_len=hparams.src_max_len,
                                               tgt_max_len=hparams.tgt_max_len, skip_count=skip_count_placeholder,
...
class EvalModel(collections.namedtuple("EvalModel", (
    "graph", "model", "src_file_placeholder", "tgt_file_placeholer", "iterator"))):
    pass
...
def create_eval_model(model_creator, hparams, scope=None, extra_args=None):
    ...
    with graph.as_default(), tf.container(scope or "eval"):
        src_vocab_table, tgt_vocab_table = vocab_utils.create_vocab_tables(src_vocab_file, 
                                                                           tgt_vocab_file, 
                                                                           hparams.share_vocab)
        src_file_placeholder = tf.placeholder(shape=(), dtype=tf.int64)
        tgt_file_placeholder = tf.placeholder(shape=(), dtype=tf.int64)
...
def _create_pretrained_embed_from_txt(vocab_file, embed_file, num_trainable_tokens=3, dtype=tf.float32, scope=None):
    ...
    embed_dict, embed_size = vocab_utils.load_embed_txt(embed_file)
    for token in trainable_tokens:
        utils.print_out(" %s" % token)
        if token not in embed_dict:
            embed_dict[token] = [0.0] * embed_size
    
    embed_matrix = np.array([embed_dict[token] for token in vocab], dtype=dtype.as_numpy_dtype())
    embed_matrix = tf.constant(embed_matrix)
    embed_matrix_const = tf.slice(embed_matrix, [num_trainable_tokens, 0], [-1, -1])

    with tf.variable_scope(scope or "pretrain_embeddings", dtype=dtype) as scope:
        with tf.device(_get_embed_device(num_trainable_tokens)):
            embed_matrix_var = tf.get_variable("embed_matrix_var", [num_trainable_tokens, embed_size])
    return tf.concat([embed_matrix_var, embed_matrix_const], 0)


def _create_or_load_embed(embed_name, vocab_file, embed_file, vocab_size, embed_size, dtype):
    if vocab_file and embed_file:
        embedding = _create_pretrained_embed_from_txt(vocab_file, embed_file)
    else:
...
def create_emb_for_encoder_and_decoder(share_vocab, src_vocab_size, tgt_vocab_size, src_embed_size,
                                       tgt_embed_size, dtype=tf.float32, num_partitions=0,
                                       src_vocab_file=None, tgt_vocab_file=None, src_embed_file=None,
                                       tgt_embed_file=None, scope=None):
    if num_partitions <= 1:
        partitioner = None
    else:
        partitioner = tf.fixed_size_partitioner(num_partitions)
    
    if (src_embed_file and tgt_embed_file) and partitioner:
        raise ValueError("Can't set num_partitions > 1 when using pretrained embeddings")
    
    with tf.variable_scope(scope or "embeddings", dtype=dtype, partitioner=partitioner) as scope:
        if share_vocab:
            if src_vocab_size != tgt_vocab_size:
                raise ValueError("Sharing embeddings but different source/target vocab sizes %d vs. %d" % (src_vocab_size, tgt_vocab_size))
            assert src_vocab_size == tgt_vocab_size
            utils.print_out("Using the same embeddings for source and target")
            ...
            with tf.variable_scope("encoder", partitioner=partitioner):
                embedding_encoder = _create_or_load_embed("embeddings_encoder", src_vocab_file, 
                                                          src_embed_file, src_vocab_size, 
                                                          src_embed_size, dtype)
            
            with tf.variable_scope("decoder", partitioner=partitioner):
                embedding_encoder = _create_or_load_embed("embeddings_decoder", tgt_vocab_file, 
                                                          tgt_embed_file, src_vocab_size, 
                                                          src_embed_size, dtype)
