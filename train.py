...
def run_sample_decode(infer_model, infer_sess, model_dir, hparams, summary_writer, src_data, tgt_data):
    with infer_model.graph.as_default():
        loaded_infer_model, global_step = model_util.create_or_load_model(infer_model.model, model_dir, infer_sess, "infer")

    _sample_decode(loaded_infer_model, global_step, infer_sess, hparams, infer_model.iterator, src_data, tgt_data,
...
def run_internal_eval(eval_model, eval_sess, model_dir, hparams, summary_writer, use_test_set=True):
    with eval_model.graph.as_default():
        loaded_eval_model, global_step = model_util.create_or_load_model(eval_model.model, model_dir, eval_sess, "eval")
...
def _external_eval(model, global_step, sess, hparams, iterator, iterator_feed_dict, tgt_file, label, summary_writer,
    ...
    scores = nmt_utils.decode_and_evaluate(label, model, sess, output, ref_file=tgt_file, metrics=hparams.metrics,
                                           subword_option=hparams.subword_option, beam_width=hparams.beam_width,
                                           tgt_eos=hparams.eos, decode=decode)
    if decode:
        for metric in hparams.metrics:
            ...
            if save_on_best and scores[metric] > getattr(hparams, best_metric_label):
                ...
                model.saver.save(sess, os.path.join(getattr(hparams, best_metric_label + "_dir"), "translate.ckpt"), global_step=model.global_step)
...
def run_external_eval(infer_model, infer_sess, model_dir, hparams, summary_writer, save_best_dev=True, 
    ...
    dev_scores = _external_eval(loaded_infer_model, global_step, infer_sess, hparams, infer_model.iterator,
                                dev_infer_iterator_feed_dict, dev_tgt_file, "dev", summary_writer,
    ...
    if use_test_set and hparams.test_prefix:
        ...
        test_scores = _external_eval(loaded_infer_model, global_step, infer_sess, hparams, infer_model.iterator,
                                     test_infer_iterator_feed_dict, test_tgt_file, "test", summary_writer,
                                     save_on_best=False, avg_ckpts=avg_ckpts)
...
def run_avg_external_eval(infer_model, infer_sess, model_dir, hparams, summary_writer, global_step):
    ...
    if hparams.avg_ckpts:
        ...
        avg_model_dir = model_util.avg_checkpoints(model_dir, hparams.num_keep_ckpts, global_step, global_step_name)

        if avg_model_dir:
            avg_dev_scores, avg_test_scores, _ = run_external_eval(infer_model, infer_sess, avg_model_dir, hparams,
                                                                   summary_writer, avg_ckpts=True)
...
def _format_results(name, perplexity, scores, metrics):
    result_str = ""
    if perplexity:
        result_str = "%s perplexity %.2f" % (name, perplexity)
    if scores:
...
def run_full_eval(model_dir, infer_model, infer_sess, eval_model, eval_sess, hparams, summary_writer, 
                  sample_src_data, sample_tgt_data, avg_ckpts=False):
    run_sample_decode(infer_model, infer_sess, model_dir, hparams, summary_writer, sample_src_data, sample_tgt_data)
    dev_perplexity, test_perplexity = run_internal_eval(eval_model, eval_sess, model_dir, hparams, summary_writer)
    ...
    avg_dev_scores, avg_test_scores = None, None
    if avg_ckpts:
        avg_dev_scores, avg_test_scores = run_avg_external_eval(infer_model, infer_sess, model_dir, hparams, summary_writer, global_step)
...
def print_step_info(prefix, global_step, info, result_summary, log_f):
    utils.print_out("%sstep %d learning_rate %g step-time %.2fs wps %.2fK perplexity %.2f grad_norm %.2f %s, %s" %
                    ...
                    info["speed"], info["train_perplexity"], info["avg_grad_norm"], result_summary, time.ctime()), log_f)
...
def train(hparams, scope=None, target_session=""):
    ...
    run_full_eval(model_dir, infer_model, infer_sess, eval_model, eval_sess, hparams, summary_writer, 
